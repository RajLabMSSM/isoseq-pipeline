...
rule SQANTI_all:
    ...
    shell:
        "ml kentutils; " # gtfToGenePred is here
        "if [ -d splits/ ];then rm -r splits/; fi; " # temp directory - if job failed then this will be left over
        "export PYTHONPATH=$PYTHONPATH:/hpc/users/humphj04/pipelines/cDNA_Cupcake/sequence/;"
                "{params.python} {params.sqantiPath}/sqanti_qc2.py "
                " -t {params.nCores} "
                " -n {params.nChunks} "
                " --dir {params.outDir} "
                " --out {params.sample} "
...
rule multiQC:
    input:
                expand("{sample}/qc/{sample}.metrics.tsv",sample = samples)
    params:
        folders = expand("{sample}/qc", sample = samples)
    output:
         "multiqc/multiqc_report.html"
    shell:
        "export LC_ALL=en_US.UTF-8; export LANG=en_US.UTF-8;"
        "multiqc -f --outdir multiqc/ {params.folders} " 
